...
class RoleService implements IRoleService
{
    ...
    public function create(array $data): IRoleDTO
    {
        ...
        if ($role)
            return new RoleDTO(null, __t('role.exists'));

        $role = $this->roleRepository->create($data);
        if (!$role)
            return new RoleDTO(null, __t('role.not_created'));

        return new RoleDTO($role, __t('role.created'));
    }
    ...
    public function get(array $fields): IRoleDTO
    {
        ...
        return new RoleDTO($found ? $role : null, $found ? __t('role.found') : __t('role.not_found'));
    }
    ...
    public function update(int $id, array $data): IRoleDTO
    {
        ...
        if (!$role)
            return new RoleDTO(null, __t('role.not_found'));

        if ($this->roleRepository->isUpToDate($role, $data))
            return new RoleDTO(null, __t('role.up_to_date'));

        $role = $this->roleRepository->update($role, $data);
        return new RoleDTO($role ?? null, $role ? __t('role.updated') : __t('role.not_updated'));
    }
    ...
    public function delete(int $id): IRoleDTO
    {
        ...
        return new RoleDTO($roleData, $role_deleted ? __t('role.deleted') : __t('role.not_deleted'));
    }
    ...
    public function assignPermission(array $data): IRoleDTO
    {
        ...
        if (!empty($missing_permissions))
            return new RoleDTO(null, __t('role.create_first', ['missing_permissions' => implode(',', $missing_permissions)]));

        $assigned_permissions = $this->roleValidator->getAlreadyAssignedPermissions($role, $data['permissions']);
        if (!empty($assigned_permissions))
            return new RoleDTO(null, __t('role.already_has_permissions', ['assigned_permissions' => implode(',', $assigned_permissions)]));
        ...
        if ($result['processed_count'] < 1)
            return new RoleDTO(null, __t('role.not_assigned'));

        return new RoleDTO($role, __t('role.successfully_assigned', ['permissions' => implode(',', $data['permissions'])]));
    }
    ...
    public function revokePermission(array $data): IRoleDTO
    {
        ...
        if (!empty($unassigned_permissions = $this->roleValidator->checkUnassignedPermissions($role, $data['permissions'])))
            return new RoleDTO(null, __t('role.has_not_permissions', ['unassigned_permissions' => implode(',', $unassigned_permissions)]));

        $permissionIds = $this->permissionRepository->getIdsByNames($data['permissions'])->toArray();
        $result = $this->roleRepository->revokePermissions($role, $permissionIds);
        if ($result['processed_count'] < 1)
            return new RoleDTO(null, __t('role.not_revoked'));

        return new RoleDTO($role, __t('role.successfully_revoked', ['permissions' => implode(',', $data['permissions'])]));
    }
