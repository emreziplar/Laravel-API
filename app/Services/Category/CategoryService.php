<?php

namespace App\Services\Category;

use App\Contracts\Category\ICategoryService;
use App\DTO\Response\BaseResponseDTO;
use App\Repositories\Contracts\Category\ICategoryRepository;

class CategoryService implements ICategoryService
{
    protected ICategoryRepository $categoryRepository;

    public function __construct(ICategoryRepository $categoryRepository)
    {
        $this->categoryRepository = $categoryRepository;
    }

    public function create(array $data): BaseResponseDTO
    {
        $parent_id = $data['parent_id'] ?? null;
        if ($parent_id)
            if (!$parent = $this->categoryRepository->getFirst($parent_id))
                return new BaseResponseDTO(null, __t('category.parent_not_found'));

        $category = $this->categoryRepository->create($data);
        if (!$category)
            return new BaseResponseDTO(null, __t('category.not_created'));

        return new BaseResponseDTO($category, __t('category.created'));
    }

    public function get(array $fields): BaseResponseDTO
    {
        $categories = $this->categoryRepository->getWithConditions($fields);

        if ($categories->isEmpty())
            return new BaseResponseDTO(null, __t('category.not_found'));

        return new BaseResponseDTO($categories, __t('category.found'));
    }

    public function update(int $id, array $data): BaseResponseDTO
    {
        $category = $this->categoryRepository->getFirst($id);
        if (!$category)
            return new BaseResponseDTO(null, __t('category.not_found'));

        $parent_id = $data['parent_id'] ?? null;

        if ($parent_id) {
            if (!$parent = $this->categoryRepository->getFirst($parent_id))
                return new BaseResponseDTO(null, __t('category.parent_not_found'));

            if ($id === $parent->getId())
                return new BaseResponseDTO(null, __t('category.not_updated_as_parent'));
        }

        $category = $this->categoryRepository->update($category, $data);
        if (!$category)
            return new BaseResponseDTO(null, __t('category.not_updated'));

        return new BaseResponseDTO($category, __t('category.updated'));
    }

    public function delete(int $id): BaseResponseDTO
    {
        $category = $this->categoryRepository->getFirst($id);
        if (!$category)
            return new BaseResponseDTO(null, __t('category.not_found'));

        $category_deleted = $this->categoryRepository->delete($category);

        $category = $category_deleted ? collect() : null; //TODO: refactor all delete responses

        return new BaseResponseDTO($category, $category_deleted ? __t('category.deleted') : __t('category.not_deleted'));
    }
}
